...
<head>
    ...
    <link rel="stylesheet" href="https://cdn.cloudcrow.com.br/TemplatesEmBreve/Template2/html/css/style.css">
    <script src="https://cdn.cloudcrow.com.br/TemplatesEmBreve/Template2/html/js/modernizr.custom.js"></script>
</head>
<style>.img-fluide{width:20em}.padd{margin-left:1em}.wrap{position:fixed;z-index:-30;top:0;left:0;overflow:hidden;width:100vw;height:100vh;margin:auto;background:linear-gradient(-141deg,{{ $gs->colors }} 0%,{{ $gs->header_color }} 100%)}@media only screen and(max-width:480px){.fullpagina .section#section0 .content-inside-section .container-inside{padding:19rem 15px}}footer{position:fixed}.fullpagina .section#section0 .content-inside-section .container-inside{position:relative;display:table;height:100vh;width:100%;margin:0 auto;padding:0 100px 80px;text-align:left}</style>
<body>
    <div id="loading">
        <div class="loader"><span class="dots">.</span><span class="dots">.</span><span class="dots">.</span><span
                class="dots">.</span><span class="dots">.</span><span class="dots">.</span></div>
    </div>
    <div class="wrap"><canvas id="liquid"></canvas></div>
    <div id="fullpage" class="fullpagina">
        <div class="section" id="section0">
            <section class="content-inside-section">
                <div class="container">
                    <div class="container-inside">
                        <div class="main-content align-center"><img src="{{ $gs->footerLogoUrl }}"
                                alt="Our company logo" class="img-fluide" style="max-width:400px;max-height:400px">
                            <h1>Em breve... <br> ...DevOps!</h1>
                            <p class="on-home">Estamos trabalhando muito para oferecer a você a melhor experiência!</p>
                            <br>
                            <div class="command">
                                <div class="col-12 col-xl-6 footer-nav">
                                    <ul class="on-right" style="font-size:2em">@php
                                        $social=App\Models\Socialsetting::find(1); @endphp @if($social->f_status==1)<a
                                            href="{{ $social->facebook }}" target="_blank"><i
                                                class="fab fa-facebook-f padd"></i></a>@endif
                                        @if($social->t_status==1)<a href="{{ $social->twitter }}" target="_blank"><i
                                                class="fab fa-twitter padd"></i></a>@endif @if($social->y_status==1)<a
                                            href="{{ $social->youtube }}" target="_blank"><i
                                                class="fab fa-youtube padd"></i></a>@endif @if($social->l_status==1)<a
                                            href="{{ $social->linkedin }}" target="_blank"><i
                                                class="fab fa-linkedin-in padd"></i></a>@endif
                                        @if($social->i_status==1)<a href="{{ $social->instagram }}" target="_blank"><i
                                                class="fab fa-instagram padd"></i></a>@endif</ul>
                                </div>
                                <div class="clear"></div>
                            </div>
                        </div>
