    ...
    <script>
        var table = $('#example').DataTable({
            ...
            initComplete: function() {
                $(".btn-area").append('<div class="col-sm-4 text-right"><a class="add-btn" data-href="{{ route('admin-cblog-create') }}" data-header="{{ __('Add New Category') }}" id="add-data" data-toggle="modal" data-target="#modal1"><i class="fas fa-plus"></i> {{ __('Add New Category') }}</a></div>');
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', 'a', function() {
                ...
                var y = x.split("/");
                if (link.attr("href").indexOf("blog") > -1 || !(link.attr("data-href") || link.attr("href").indexOf("#") > -1 || link.attr("href").indexOf("javascript") > -1 || link.attr("href").indexOf(y[1]) > -1)) {
                    sessionStorage.setItem("CurrentPage", 0);
                    table.state.clear();
                }
            });
