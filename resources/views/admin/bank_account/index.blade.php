    ...
    <div class="row">
      <div class="col-lg-12">
        ...
        <ul class="links">
          ...
          <li><a href="{{ route('admin-bank_account-index') }}">{{ __('Bank Accounts') }}</a></li>
        </ul>
      </div>
    </div>
  </div>
            ...
            <table id="example" class="table table-hover dt-responsive" cellspacing="0" width="100%">
              <thead>
                <tr>
                  ...
                  <th>{{ __('Name') }}</th>
                  <th><i class="fa fa-info-circle fa-lg" title='{{ __('Info') }}'></i></th>
                  <th><i class="fa fa-eye fa-lg" title='{{ __('Status') }}'></i></th>
                </tr>
              </thead>
            </table>
          </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content text-center">
      ...
      <div class="modal-body">
        <p>{{ __('You are about to delete this Bank Account.') }}</p>
...
$(document).ready(function() {
    ...
    initComplete: () => {
      $(".btn-area").append(`
        <div class="col-sm-4 table-contents">
          <a class="add-btn" data-href="{{ route('admin-bank_account-create') }}" data-header="{{ __('Add New Bank Account') }}" data-toggle="modal" data-target="#modal1">
            <i class="fas fa-plus"></i> <span class="remove-mobile">{{ __('Add New Bank Account') }}</span>
          </a>
        </div>`);
      table.on('page.dt', () => sessionStorage.setItem("CurrentPage", table.page()));
    }
  });
  ...
  $(document).on('click', 'a', function() {
    const link = $(this);
    const prefix = '{{ Request::route()->getPrefix() }}'.split("/")[1];
    if (!(link.data("href") || link.attr("href").includes("#") || link.attr("href").includes("javascript") || link.attr("href").includes(prefix))) {
