                                                ...
                                                <div class="">
                                                        ...
                                                        <div class="panel-footer">
                                                            <ul class="nav nav-pills" role="tablist">
                                                                <li role="presentation" class="active">
                                                                    <a :href="'#{{$lang->locale}}-optionfield'+n" class="active" :aria-controls="'{{$lang->locale}}-optionfield'+n" role="tab" data-toggle="tab">{{$lang->language}}</a>
                                                                </li>
                                                                @foreach ($locales as $loc)
                                                                    @if($loc->locale === $lang->locale) @continue @endif
                                                                    <li role="presentation">
                                                                        <a :href="'#{{$loc->locale}}-optionfield'+n" :aria-controls="'{{$loc->locale}}-optionfield'+n" role="tab" data-toggle="tab">{{$loc->language}}</a>
                                                                    </li>
                                                                @endforeach
                                                            </ul>
                                                        </div>
                                                    ...
                                                    <div class="panel panel-lang">
                                                        ...
                                                        <div class="panel-footer">
                                                            <ul class="nav nav-pills" role="tablist">
                                                                <li role="presentation" class="active">
                                                                    <a :href="'#{{$lang->locale}}-descriptionfield'+n" class="active" :aria-controls="'{{$lang->locale}}-descriptionfield'+n" role="tab" data-toggle="tab">{{$lang->language}}</a>
                                                                </li>
                                                                @foreach ($locales as $loc)
                                                                    @if($loc->locale === $lang->locale) @continue @endif
                                                                    <li role="presentation">
                                                                        <a :href="'#{{$loc->locale}}-descriptionfield'+n" :aria-controls="'{{$loc->locale}}-descriptionfield'+n" role="tab" data-toggle="tab">{{$loc->language}}</a>
                                                                    </li>
                                                                @endforeach
                                                            </ul>
                                                        </div>
...
var app = new Vue({
    ...
    methods: {
        ...
        removeOption(n) {
            if($(".counterrow").length >= 2) { $("#counterrow"+n).remove(); }
            if ($(".counterrow").length == 0) { this.counter = 0; }
        }
    }
})
</script>
