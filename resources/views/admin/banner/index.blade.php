...
{{-- Modals --}}
@foreach (['modal1', 'confirm-delete'] as $modal)
<div class="modal fade" id="{{ $modal }}" tabindex="-1" role="dialog" aria-labelledby="{{ $modal }}" aria-hidden="true">
    <div class="modal-dialog{{ $modal == 'modal1' ? ' modal-dialog-centered' : '' }}">
        <div class="modal-content">
            @if($modal == 'modal1')
            <div class="submit-loader"><img src="{{ $admstore->adminLoaderUrl }}" alt=""></div>
            @endif
            <div class="modal-header{{ $modal == 'confirm-delete' ? ' text-center' : '' }}">
                <h{{ $modal == 'modal1' ? 5 : 4 }} class="modal-title">{{ $modal == 'confirm-delete' ? __('Confirm Delete') : '' }}</h{{ $modal == 'modal1' ? 5 : 4 }}>
                <button type="button" class="close" data-dismiss="modal">&times;</button>
            </div>
            <div class="modal-body{{ $modal == 'confirm-delete' ? ' text-center' : '' }}">
                @if($modal == 'confirm-delete')
                    <p>{{ __('You are about to delete this Banner.') }}</p>
                    <p>{{ __('Do you want to proceed?') }}</p>
                @endif
            </div>
            <div class="modal-footer{{ $modal == 'confirm-delete' ? ' justify-content-center' : '' }}">
                <button type="button" class="btn btn-{{ $modal == 'confirm-delete' ? 'default' : 'secondary' }}" data-dismiss="modal">
                    {{ __('Close') }}
                </button>
                @if($modal == 'confirm-delete') <a class="btn btn-danger btn-ok">{{ __('Delete') }}</a> @endif
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        sessionStorage.getItem('SelectedStoreFilter') 
