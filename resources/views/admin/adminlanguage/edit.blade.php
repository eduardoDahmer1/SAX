...
@section('styles')
    <style type="text/css">textarea.input-field { padding: 20px 20px 0; border-radius: 0; }</style>
@endsection
            ...
            <div class="row">
                <div class="col-lg-12">
                    ...
                    <ul class="links">
                        ...
                        <li><a href="{{ route('admin-tlang-edit', $data->id) }}">{{ __('Edit') }}</a></li>
                    </ul>
                </div>
            </div>
        </div>
                                                        ...
                                                        <table id="geniustable" class="table table-hover dt-responsive" cellspacing="0" width="100%">
                                                            ...
                                                            <tbody>
                                                                @foreach ($langEdit as $key => $val)
                                                                    @php if (is_numeric($key)): continue; endif; @endphp
                                                                    <tr>
    ...
    <script type="text/javascript">
        var table = $('#geniustable').DataTable({
            ...
            initComplete: function(settings, json) {
                $(".btn-area").append('<div class="col-sm-4 table-contents"><a class="add-btn" id="btn-empty-search">{{ __('Show Empty') }}</a><a class="add-btn" id="btn-all-search" style="display: none;">{{ __('Show All') }}</a></div>');
                $('#btn-empty-search').on('click', function() {
                    var regex = "^$";
                    table.column(2).search(regex, true, false).draw();
                    $('#btn-empty-search').hide();
                    $('#btn-all-search').show();
                });
                $('#btn-all-search').on('click', function() {
                    table.column(2).search("").draw();
                    $('#btn-all-search').hide();
                    $('#btn-empty-search').show();
                });
            }
        });
    </script>
