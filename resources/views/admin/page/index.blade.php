        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <ul class="links">
                    ...
                    <li><a href="{{ route('admin-page-index') }}">{{ __('Pages') }}</a></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    var table = $('#geniustable').DataTable({
        ...
        drawCallback: function(settings) {
            ...
            $(".checkboxHeader").on('click', function() {
                ...
                var headerNovo = ($(this).attr('name').slice(-1) == "0") ? "1" : "0";
                $.ajax({type: 'GET', url: '{{ url('admin/page/header') }}' + '/' + id + '/' + headerNovo});
            });
            $(".checkboxFooter").on('click', function() {
                var id = $(this).attr("id").replace("checkbox-footer-", "");
                var footerNovo = ($(this).attr('name').slice(-1) == "0") ? "1" : "0";
                $.ajax({type: 'GET', url: '{{ url('admin/page/footer') }}' + '/' + id + '/' + footerNovo});
            });
        },
        initComplete: function(settings, json) {
            $(".btn-area").append('<div class="col-sm-4 text-right"><a class="add-btn" data-href="{{ route('admin-page-create') }}" data-header="{{ __('Add New Page') }}" id="add-data" data-toggle="modal" data-target="#modal1"><i class="fas fa-plus"></i> {{ __('Add New Page') }}</a></div>');
