    ...
    <script>
        var table = $('#geniustable').DataTable({
            ...
            columns: [
                ...
                { data: 'name', render: (data, type, row) => row.name + '<br><small> slug: ' + row.slug + '</small>' },
            ...
            drawCallback: function() {
                ...
                $(".checkboxStatus").on('click', function() {
                    ...
                    var statusNovo = (this.name.slice(-1) === "0") ? "1" : "0";
                    $.ajax({ type: 'GET', url: '{{ url('admin/category/status') }}' + '/' + id + '/' + statusNovo });
                });
                $('.presentation-pos').on('change', function() {
                    var id = $(this).data('cat');
                    var pos = Math.abs($(this).val());
                    $.ajax({ type: 'GET', url: '{{ url('admin/category/changeCatPos') }}' + '/' + id + '/' + pos });
