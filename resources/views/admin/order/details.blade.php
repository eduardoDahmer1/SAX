        ...
        <div class="col-lg-6">
            <div class="special-box">
                    ...
                    <table class="table">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td width="10%">:</td>
                                <td width="45%">{{ $order->currency_sign }}{{ number_format($order->shipping_cost * $order->currency_value, $order_curr->decimal_digits, $order_curr->decimal_separator, $order_curr->thousands_separator) }}</td>
                            </tr>
                            @endif
                            ...
                            <tr>
                                ...
                                <td width="10%">:</td>
                                <td width="45%">{{ $order->currency_sign }}{{ number_format($order->packing_cost * $order->currency_value, $order_curr->decimal_digits, $order_curr->decimal_separator, $order_curr->thousands_separator) }}</td>
                            </tr>
                            @endif
                            <tr>
                                <th width="45%">{{ __('Total Cost') }}</th>
                                <td width="10%">:</td>
                                <td width="45%">{{ $order->currency_sign }}{{ number_format($order->pay_amount * $order->currency_value, $order_curr->decimal_digits, $order_curr->decimal_separator, $order_curr->thousands_separator) }}</td>
                            </tr>
                            <tr>
                                <th width="45%">{{ $first_curr->sign . ' ' . __('Total:') }}</th>
                                <td width="10%">:</td>
                                <td width="45%">{{ $first_curr->sign }}{{ number_format($order->pay_amount, $first_curr->decimal_digits, $first_curr->decimal_separator, $first_curr->thousands_separator) }}</td>
                            </tr>
                            <tr>
                                <th width="45%">{{ __('Ordered Date') }}</th>
                                <td width="10%">:</td>
                                <td width="45%">{{ $order->created_at->formatLocalized('%d-%b-%Y %T') }}</td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <th width="10%">:</th>
                                <td width="45%"><span>{{$order->billing ? __('Yes') : __('No')}}</span></td>
                            </tr>
                            <tr>
                                <th width="45%">{{__('CEC Number')}}</th>
                                <th width="10%">:</th>
                                <td width="45%"><span>{{$order->number_cec}}</span></td>
                            </tr>
                ...
                <div class="footer-area">
                    ...
                    <br><a href="{{ route('admin-order-receipt', $order->id) }}" class="mybtn1"><i class="fas fa-search-dollar"></i> {{ __('View Receipt') }}</a>
                    @elseif($order->method == 'Paghiper')
                    <br><a href="{{ route('admin-order-billet-status', $order->id) }}" class="mybtn1"><i class="fas fa-search-dollar"></i> {{ __('Check Billet Status') }}</a>
                    @endif
                    @if ($admstore->is_melhorenvio && config('features.melhorenvio_shipping'))
                    <a id="request-melhorenvio" href="{{ route('admin-order-confirm-melhorenvio-package', $order->id) }}" class="mybtn1 ml-5"><i class="fas fa-shipping-fast"></i>{{ __('Request Melhor Envio') }}</a>
                    @endif
                    @if ($admstore->is_aex && config('features.aex_shipping'))
                    <a id="request-aex" href="{{ route('admin-order-select-aex-city', $order->id) }}" class="mybtn1 ml-5"><i class="fas fa-shipping-fast"></i>{{ __('Request AEX') }}</a>
                    @endif
                </div>
            </div>
        </div>
                        ...
                        <table class="table">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <th width="10%">:</th>
                                    <td width="45%">{{ $order->customer_address_number }}</td>
                                </tr>
                                <tr>
                                    <th width="45%">{{ __('Customer complement') }}</th>
                                    <th width="10%">:</th>
                                    <td width="45%">{{ $order->customer_complement }}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td width="45%">
                                        @if ($admstore->currency_format == 0)
                                        {{ $order->currency_sign }}{{ $order->affilate_charge }}
                                        @else
                                        {{ $order->affilate_charge }}{{ $order->currency_sign }}
                                        @endif
                                    </td>
                                </tr>
                                @endif
                        ...
                        <table class="table">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <th width="10%">:</th>
                                    <td width="45%">{{ $order->shipping_address_number == null ? $order->customer_address_number . " " . $order->customer_complement : $order->shipping_address_number . " " . $order->shipping_complement }}</td>
                                </tr>
                                <tr>
            ...
            <div class="col-lg-6">
                <div class="special-box">
                    <div class="heading-area">
                        <h4 class="title">{{ __('Custom Products') }}</h4>
                    </div>
                    <div class="table-responsive-sm">
                        <table class="table">
                            <tbody>
                                <tr>
                                    ...
                                    <th width="10%">:</th>
                                    <td><a target="_blank" href="{{ route('front.product', $product['item']['slug']) }}">{{ mb_strlen($product['item']['name'], 'utf-8') > 30 ? mb_substr($product['item']['name'], 0, 30, 'utf-8') . '...' : $product['item']['name'] }}</a></td>
                                </tr>
                                <tr>
                                    <th width="45%"><strong>{{ __('Product SKU') }}:</strong></th>
                                    <th width="10%">:</th>
                                    @php $prod = App\Models\Product::find($product['item']['id']); @endphp
                                    @if (isset($prod))<td>{{ $prod->sku }}</td>@endif
                                </tr>
                                ...
                                <tr class="mb-2">
                                    ...
                                    <td width="45%">
                                        <a href="{{ route('admin-customprod-download', $product['customizable_logo']) }}" class="{{ isset($product['customizable_logo']) ? '' : 'collapse' }}">
                                            <button class="btn btn uploadLogoBtn"><p><i class="fa fa-download"></i> {{ __('Download File') }}</p></button>
                                        </a>
                                    </td>
                                </tr>
                                @endif
                                    ...
                                    <td>
                                        <div style="display:inline-block;">
                                            <div style="position:relative;display:flex;justify-content:center;align-items:center;">
                                                @if (isset($gal))<span class="galleryId">{{ $gal->id }}</span>@endif
                                                <img src="{{ asset('storage/images/galleries/' . $product['customizable_gallery']) }}" style="width:60px;border-radius:30px;" alt="">
                                            </div>
                                        </div>
                                    </td>
                                </tr>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        @if ($product['license'] != '')<a href="javascript:;" data-toggle="modal" data-target="#confirm-delete" class="btn btn-info product-btn" id="license" style="padding: 5px 12px;"><i class="fa fa-eye"></i> {{ __('View License') }}</a>@endif
                                        @php $prod = App\Models\Product::find($product['item']['id']); @endphp
                                        @if (isset($prod))
                                    ...
                                    <td>
                                        ...
                                        @if (isset($product['material']) && !empty($product['material']))<p><strong>{{ __('Material') }}: </strong>{{ str_replace('-', ' ', $product['material']) }}</p>@endif
                                        <p><strong>{{ __('Price') }}:</strong> {{ $order->currency_sign }}{{ number_format($product['item']['price'] * $order->currency_value, $order_curr->decimal_digits, $order_curr->decimal_separator, $order_curr->thousands_separator) }}<small><br>{{ $first_curr->sign . ' ' . __('Price') }}: {{ $first_curr->sign }}{{ number_format($product['item']['price'], $first_curr->decimal_digits, $first_curr->decimal_separator, $first_curr->thousands_separator) }}</small></p>
                                        <p><strong>{{ __('Qty') }}:</strong> {{ $product['qty'] }} {{ $product['item']['measure'] }}</p>
                                        @if (!empty($product['keys']))@foreach (array_combine(explode(',', $product['keys']), explode('~', $product['values'])) as $key => $value)<p><b>{{ App\Models\Attribute::where('input_name', $key)->first()->name }}:</b> {{ $value }}</p>@endforeach @endif
                                    </td>
                                    <td style="text-align: left;">{{ $order->currency_sign }}{{ number_format($product['price'] * $order->currency_value, $order_curr->decimal_digits, $order_curr->decimal_separator, $order_curr->thousands_separator) }}<br><small>{{ $first_curr->sign }}{{ number_format($product['price'], $first_curr->decimal_digits, $first_curr->decimal_separator, $first_curr->thousands_separator) }}</small></td>
                                </tr>
                                @endforeach
                                @endif
                            </tbody>
                        ...
                        <table id="melhorenvio_table" class="table table-hover dt-responsive" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center;">{{ $request->service->company->name }}<br><small>{{ $request->service->name }}</small></td>
                                    ...
                                    <td style="text-align: center;">
                                        @if ($request->status == 'pending')<span class="badge badge-secondary">{{ __('Pending') }}</span>
                                        @elseif($request->status == 'released')<span class="badge badge-info">{{ __('Released') }}</span>
                                        @elseif($request->status == 'posted')<span class="badge badge-primary">{{ __('Posted') }}</span>
                                        @elseif($request->status == 'delivered')<span class="badge badge-success">{{ __('Delivered') }}</span>
                                        @elseif($request->status == 'canceled')<span class="badge badge-danger">{{ __('Canceled') }}</span>
                                        @elseif($request->status == 'undelivered')<span class="badge badge-warning">{{ __('Undelivered') }}</span>
                                        @endif
                                    </td>
                                    <td style="text-align: center;">
                                        <div>
                                            @php $disable = !empty($request->posted_at) || !empty($request->canceled_at) || !empty($request->expired_at) || !empty($request->delivered_at); @endphp
                                            <a class="btn {{ empty($request->preview_url) ? ' disabled' : '' }}" href="{{ $request->preview_url }}" target="_blank" data-toggle="tooltip" title="{{ __('Preview') }}"><i class="fas fa-eye h5"></i></a>
                                            <a class="btn {{ !empty($request->generated_at) || $disable ? ' disabled' : '' }}" href="{{ route('admin-order-generate-melhorenvio', $request->id) }}" data-toggle="tooltip" title="{{ __('Generate') }}"><i class="fas fa-check-circle h5"></i></a>
                                            <a class="btn {{ empty($request->generated_at) || empty($request->print_url) || $disable ? 'disabled' : '' }}" href="{{ $request->print_url }}" target="_blank" data-toggle="tooltip" title="{{ __('Print') }}"><i class="fas fa-print h5"></i></a>
                                            <a class="btn {{ $disable ? ' disabled' : '' }}" href="{{ route('admin-order-cancel-melhorenvio', $request->id) }}" data-toggle="tooltip" title="{{ __('Cancel') }}"><i class="fas fa-trash-alt h5"></i></a>
                                        </div>
                                    </td>
                                </tr>
                                @endforeach
