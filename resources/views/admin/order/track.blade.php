        ...
        <div class="col-lg-12">
            <div class="product-description">
                <div class="body-area">
                    ...
                    <form id="trackform" action="{{ route('admin-order-track-store') }}" method="POST" enctype="multipart/form-data">{{ csrf_field() }}
                        @include('includes.admin.form-both')
                        <input type="hidden" name="order_id" value="{{ $order->id }}">
                        <div class="row">
                            <div class="col-xl-12"><div class="input-form">@component('admin.components.input-localized', ['required' => true, 'type' => 'textarea']) @slot('name') title @endslot @slot('placeholder') {{ __('Title') }} @endslot {{ __('Title') }} *@endcomponent</div></div>
                            <div class="col-xl-12"><div class="input-form">@component('admin.components.input-localized', ['type' => 'textarea']) @slot('name') text @endslot @slot('placeholder') {{ __('Details') }} @endslot {{ __('Details') }} @endcomponent</div></div>
                        </div>
                    ...
                    <div class="body-area">
                        <div class="table-responsive show-table ml-3 mr-3">
                            <table class="table" id="track-load" data-href={{ route('admin-order-track-load', $order->id) }}>
                                <tr><th>{{ __('Title') }}</th><th>{{ __('Details') }}</th><th>{{ __('Date') }}</th><th>{{ __('Time') }}</th><th>{{ __('Options') }}</th></tr>
                                @foreach ($order->tracks as $track)
                                <tr data-id="{{ $track->id }}">
                                    <td width="30%" class="t-title"><span data-locale="{{ $lang->locale }}">{{ $track->title }}</span>@foreach ($locales as $loc) @if ($loc->locale === $lang->locale) @continue @endif <span data-locale="{{ $loc->locale }}" class="d-none">{{ $track->translate($loc->locale)->title ?? $track->title }}</span> @endforeach</td>
                                    <td width="30%" class="t-text"><span data-locale="{{ $lang->locale }}">{{ $track->text ?? '' }}</span>@foreach ($locales as $loc) @if ($loc->locale === $lang->locale) @continue @endif <span data-locale="{{ $loc->locale }}" class="d-none">{{ $track->translate($loc->locale)->text ?? $track->text }}</span> @endforeach</td>
