...
<style>
    ...
    .mr-breadcrumb .links .action-list .go-dropdown-toggle { padding: 0 30px 0 20px; }
</style>
@endsection
@section('content')
        ...
        <div class="row">
            <div class="col-lg-12">
                ...
                <ul class="links">
                    ...
                    <li><a href="{{ route('admin-gs-fedexconf') }}">{{ __('Fedex Shipping') }}</a></li>
                ...
                <div class="product-description">
                    <div class="body-area">
                        ...
                        <form action="{{ route('admin-gs-update-fedex') }}" id="geniusform" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="col-lg-6">
                                    <div class="action-list">
                                        <select class="process select droplinks {{ $admstore->fedex->production == 1 ? 'drop-success' : 'drop-danger' }}">
                                            <option data-val="1" value="{{ route('admin-gs-fedex-production', 1) }}" {{ $admstore->fedex->production == 1 ? 'selected' : '' }}>{{ __('Activated') }}</option>
                                            <option data-val="0" value="{{ route('admin-gs-fedex-production', 0) }}" {{ $admstore->fedex->production == 0 ? 'selected' : '' }}>{{ __('Deactivated') }}</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            <div class="row justify-content-center">
                                <div class="col-lg-3"><div class="left-area"><h4 class="heading">{{ __('Client id') }}</h4></div></div>
                                <div class="col-lg-6"><input name="client_id" class="input-field" placeholder="{{ __('e.g XXXX-XXX-XXXX-XXX') }}" value="{{ isset($fedex->client_id) ? $fedex->client_id : '' }}"></div>
                            ...
                            <div class="row justify-content-center">
                                <div class="col-lg-3"><div class="left-area"><h4 class="heading">{{ __('Client secret') }}</h4></div></div>
                                <div class="col-lg-6"><input name="client_secret" class="input-field" placeholder="{{ __('e.g XXXX-XXX-XXXX-XXX') }}" value="{{ isset($fedex->client_secret) ? $fedex->client_secret : '' }}"></div>
