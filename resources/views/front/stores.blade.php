    ...
    <div class="container">
        <div class="row">
            @if (count($vendor_stores) > 0)
                ...
                @foreach ($vendor_stores as $store)
                    @php $vendor_storestart = \App\Helpers\Helper::strNormalize(strtoupper(mb_substr($store->shop_name, 0, 1))); @endphp
                    @if(is_numeric($vendor_storestart)) @php $vendor_storestart = '#'; @endphp @endif
                    @if($vendor_storestart !== $currentChar)
                        @php $currentChar = $vendor_storestart; @endphp
                        <div id="title-brands" class="section-top">
                            <h2 class="section-title" id="brands-starting-with-{{($currentChar === '#' ? 'numbers' : $currentChar)}}">{{$currentChar}}</h2>
                        </div>
                    @endif
                    <div class="col-xl-2 col-lg-3 col-md-4 sc-common-padding d-flex flex-column">
                        <a href="{{ route('front.vendor', str_replace(' ', '-', $store->shop_name)) }}" class="single-category">
                            <div class="left"><h5 class="title">{{ $store->shop_name }}</h5></div>
                            <div class="right"><img src="{{ $store->vendorPhoto }}" alt="{{$store->shop_name}}"></div>
                        </a>
                    </div>
                @endforeach
            @else
                <div class="col-lg-12">
                    <div class="page-center"><h4 class="text-center">{{ __("No Stores Found.") }}</h4></div>
                </div>
            @endif
        </div>
    </div>
