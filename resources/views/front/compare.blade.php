        ...
        <div class="row">
            <div class="col-lg-12">
                <ul class="pages">
                    ...
                    <li><a href="{{ route('product.compare') }}">{{ __("Compare") }}</a></li>
                </ul>
            </div>
        </div>
    </div>
            ...
            <div class="col-lg-12">
                <div class="content">
                    <div class="com-heading">
                        <h2 class="title">{{ __("Product Compare") }}</h2>
                    </div>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="product-image-title c{{$product['item']['id']}}">
                                            <img class="img-fluid" src="{{ filter_var($product['item']['thumbnail'], FILTER_VALIDATE_URL) ? $product['item']['thumbnail'] : asset('storage/images/thumbnails/'.$product['item']['thumbnail']) }}" alt="{{ $product['item']->name }}">
                                            <a href="{{ route('front.product', $product['item']['slug']) }}">
                                                <h4 class="title">{{ $product['item']->name }}</h4>
                                            </a>
                                        </td>
                                        @endforeach
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        @foreach($products as $product)
                                        <td class="pro-desc c{{$product['item']['id']}}"><p>{{ strip_tags($product['item']->details) }}</p></td>
                                        @endforeach
                                    </tr>
                                    @if($gs->is_cart)
                                    <tr>
