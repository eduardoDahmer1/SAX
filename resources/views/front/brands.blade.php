            ...
            @if(count($brands) > 0)
                ...
                @foreach ($brands as $brand)
                    @php
                        ...
                        $hasValidImage = !empty($brand->thumbnail) && strpos($brand->thumbnail, 'noimage.png') === false;
                    @endphp

                    @if($hasProducts && $hasValidImage)
                        ...
                        @if($brandStart !== $currentChar)
                            ...
                            <div id="title-brands" class="section-top">
                                <h2 class="section-title" id="brands-starting-with-{{ $currentChar === '#' ? 'numbers' : $currentChar }}">
                                    {{ $currentChar }}
                                </h2>
                            </div>
                        @endif
