...
<div class="menufixed">
        ...
        <div class="container">
            <div class="row justify-content-between align-items-center align-items-lg-end" style="position: relative;">
                <div class="col-lg-2 col-2 order-2 order-lg-1" style="position: static;">
                    <div class="button-open-search"><input type="checkbox"><i class="fa fa-search" aria-hidden="true"></i></div>
                    <div class="search-box-wrapper">
                        <div class="search-box">
                            <form id="searchForm" class="search-form" action="{{ route('front.category') }}"
                                ...
                                @endif
                                <input type="text" id="prod_name" name="searchHttp" aria-label="Search"placeholder="{{ __('What are you looking for?') }}"value="{{ request()->input('searchHttp') }}" autocomplete="off">
                    ...
                    <div class="helpful-links">
                        <ul class="helpful-links-inner">
                            ...
                            title="{{ !Auth::guard('web')->check() ? __('Login') :  __('Profile') }}" >
                                @if (!Auth::guard('web')->check())<a href="{{ route('user.login') }}" class="profile carticon"><img class="img-fluid icons-header" src="{{ asset('assets/images/theme15/iconelogin.png')}}" alt=""></a>
                                @else
                                    <a href="{{ route('user-dashboard') }}" class="profile carticon"><img class="img-fluid icons-header" src="{{ asset('assets/images/theme15/iconeloginverify.png')}}" alt=""></a>
                                @endif
                            </li>
                            @if (!env('ENABLE_SAX_BRIDAL'))
                                <li class="wishlist" data-toggle="tooltip" data-placement="top"title="{{ __('Wish') }}">
                                    @if (Auth::guard('web')->check())
                                        <a href="{{ route('user-wishlists') }}" class="wish"><img class="img-fluid icons-header" src="{{ asset('assets/images/theme15/wishicone.png')}}" alt=""><span id="wishlist-count">{{ count(Auth::user()->wishlists) }}</span></a>
                                    @else
                                        <a href="javascript:;" data-toggle="modal" id="wish-btn"data-target="#comment-log-reg" class="wish"><img class="img-fluid icons-header" src="{{ asset('assets/images/theme15/wishicone.png')}}" alt=""><span id="wishlist-count">0</span></a>
                                    @endif
                                </li>
                                @if ($gs->is_cart)
                                    <li class="my-dropdown"><a href="javascript:;" class="cart carticon"><div class="icon"><img class="img-fluid icons-header" src="{{ asset('assets/images/theme15/bagicone.png')}}" alt=""><span class="cart-quantity" id="cart-count">{{ Session::has('cart') ? count(Session::get('cart')->items) : '0' }}</span></div></a>
                                        <div class="my-dropdown-menu" id="cart-items">@include('load.cart')</div>
                                    </li>
                                @endif
                            @else
                                <li class="my-dropdown"><a href="javascript:;" class="cart carticon"><div class="icon"><img class="img-fluid icons-header" src="{{ asset('assets/images/theme15/wishicone.png')}}" alt=""><span class="cart-quantity" id="cart-count">{{ Session::has('cart') ? count(Session::get('cart')->items) : '0' }}</span></div></a>
                ...
                <div class="col-lg-12 col-2 order-1 order-lg-4">
                    <div class="saxnavigation">
                        ...
                        <ul class="menu-navigation">
                            <li><a class="navlink" href="{{ route('front.index') }}">{{ __('Home') }}</a></li>
                            <li class="menudrop"><a  class="navlink" href="{{ route('front.categories') }}">{{ __('Categories') }}<svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="15px" height="15px" viewBox="0 0 201.457 201.457"xml:space="preserve"><g><path style="fill:currentColor;" d="M46.233,8.279L54.513,0l100.729,100.723L54.495,201.457l-8.279-8.28l92.46-92.46L46.233,8.279z"/></g></svg></a></li>
                            ...
                            @else
                                <li class="login-button-menu"><a class="navlink" href="{{ route('user.login') }}">{{ __('Login') }}</a></li>
                            @endif
                            @if ($gs->is_contact == 1)
                                <li><a  class="navlink" href="{{ route('front.contact') }}">{{ __('Contact Us') }}</a></li>
                            @endif
                        </ul>
                    </div>
                </div>
    ...
    {{-- <div class="mainmenu-area mainmenu-bb">
        <div class="container">
            <div class="row mainmenu-area-innner">
                <div class="col-6 col-lg-10 d-flex justify-content-center align-items-center">
                    <div class="categories_menu vertical">
                        <div class="categories_title">
                            <h2 class="categori_toggle"><i class="fas fa-layer-group"></i> {{ __('Categories') }}<i class="fa fa-angle-down arrow-down"></i></h2>
                        </div>
                            ...
                            <ul style="width:100%;">
                                ...
                                @foreach ($categories as $category)
                                    ...
                                    <li class="{{ $count ? 'dropdown_list' : '' }} {{ $i >= 15 ? 'rx-child' : '' }} qntd">
                                        @if ($count)
                                            @if ($category->photo)
                                                <div class="img"><img src="{{ asset('storage/images/categories/' . $category->photo) }}"alt=""></div>
                                            @endif
                                            <div class="link-area">
                                        ...
                                        @else
                                            <a href="{{ route('front.category', $category->slug) }}">
                                                @if ($category->photo)<imgsrc="{{ asset('storage/images/categories/' . $category->photo) }}">
                                                @endif
                                                {{ $category->name }}
                                            </a>
                                        @endif
                                    ...
                                    @endphp
                                    @if ($i == 15)
                                        <li><a href="{{ route('front.categories') }}"><i class="fas fa-plus"></i>{{ __('See All Categories') }}</a></li>
                                    @break
                                @endif
                            @endforeach
                        </ul>
                ...
                <div class="categories_menu horizontal">
                    <div class="categories_title_horizontal"><h2 class="categori_toggle"><i class="fa fa-bars"></i> {{ __('Categories') }}<i class="fa fa-angle-down arrow-down"></i></h2></div>
        ...
        <div class="col-6 col-lg-2"><div class="box-button-site" data-menu-toggle-main="#menu-browse-site"><i class="fas fa-bars"></i><p>{{ __('Browse the site') }}</p><div id="menu-browse-site" class="container-menu"></div></div></div>
    </div> --}}
</div>
</div>
</div>
