                ...
                <div class="fotter-social-links mt-3">
                    ...
                    <ul class="d-flex">
                        @foreach (['facebook', 'instagram', 'twitter', 'linkedin', 'dribble', 'youtube'] as $platform)
                            @if ($socials->{$platform[0] . '_status'} == 1)
                                <li>
            ...
            <div class="row">
                <div class="col-lg-12">
                    <p class="text-center m-0">
                        <small class="text-white">{{ $gs->title }} © {{ date('Y') }}.
                        {{ $gs->company_document ? '| ' . $gs->document_name . ' - ' . $gs->company_document . ' |' : '' }}
                        {{ __('All Rights Reserved') }}.</small>
                    </p>
                    <p class="text-center m-0">
                        <small class="text-white">{{ __('Developed By') }} <a id="agcrow" href="https://crowtech.digital/">CrowTech</a></small>
                    </p>
                </div>
            </div>
        </div>
