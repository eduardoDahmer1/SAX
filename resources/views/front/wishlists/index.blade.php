...
@section('content')
<style>a.wishlist:hover {text-decoration: underline;}</style>
            ...
            <div class="modal-body">
                <form method="POST" action="{{ route('user-wishlists.store') }}">
                    ...
                    <div class="row justify-content-end px-3">
                        ...
                        <button type="submit" class="btn btn-style-1 px-2 py-1">
...
<div class="container-fluid mt-5 mb-5">
    <div class="row px-5">
        <div class="col-12 d-flex justify-content-between py-3">
            <h1 class="h3 mb-3">{{ __('Your wishlists') }}</h1>
        ...
        @foreach ($wishlistsGroup as $group)
            <a class="col-12 py-3 px-2 align-center wishlist border-top" href="{{ route('user-wishlists.show', $group) }}">
                <div class="row">
                    <div class="col-md-1 col-3">
                        @if ($group->wishlists()->count())
                            <img src="{{ $group->wishlists()->first()->product->image }}" alt="{{ $group->name }} Image">
                        @endif
                    </div>
                    ...
                    <div class="col-1 d-flex flex-column justify-content-center">
                        <form action="{{ route('user-wishlists.destroy', $group->id) }}" method="POST">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-light border" type="submit" aria-label="{{ __('Delete') }}"><i class="fas fa-trash"></i></button>
                        </form>
                    </div>
                </div>
            </a>
