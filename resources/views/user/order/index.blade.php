        ...
        <div class="row">
            ...
            <div class="col-lg-8">
                <div id="div-table" class="user-profile-details">
                    <div class="order-history">
                        <div class="header-area">
                            <h4 class="title">{{ __('Pending Receipt Orders') }}</h4>
                        </div>
                                        ...
                                        @foreach ($orders as $order)
                                            @if ($order->method == 'Bank Deposit' && $order->payment_status == 'Pending' && $order->status != 'declined')
                                                ...
                                                <tr>
                                                    ...
                                                    <td><span class="badge badge-warning"><a href="{{ route('user-upload-receipt', $order->id) }}">{{ __('Upload your receipt') }}</a></span></td>
                                                    <td><a href="{{ route('user-order', $order->id) }}">{{ __('VIEW ORDER') }}</a></td>
                                                </tr>
                                            @endif
                                        @endforeach
                                    </tbody>
                ...
                <div class="user-profile-details">
                    <div class="order-history">
                        <div class="header-area">
                            <h4 class="title">{{ __('Purchased Items') }}</h4>
                        </div>
                        <div class="mr-table allproduct mt-4">
                            <div class="table-responsiv">
                                    ...
                                    <tbody>
                                        @foreach ($orders as $order)
                                            ...
                                            <tr>
                                                ...
                                                <td>{{ date('d M Y', strtotime($order->created_at)) }}</td>
                                                <td>{{ $order->currency_sign }}{{ number_format($order->pay_amount * $order->currency_value, $order_curr->decimal_digits, $order_curr->decimal_separator, $order_curr->thousands_separator) }}</td>
                                                <td><div class="payment-status {{ lcfirst($order->payment_status) }}">{{ __(ucfirst($order->payment_status)) }}</div></td>
                                                <td><div class="order-status {{ $order->status }}">{{ __(ucfirst($order->status)) }}</div></td>
                                                <td><a href="{{ route('user-order', $order->id) }}">{{ __('VIEW ORDER') }}</a></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
